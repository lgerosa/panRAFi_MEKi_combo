...
if (unique_cld_color){
  ...
  cell_line_color_grad[["IPC-298"]] <- c("#3d0067","#9900ff", "#d79aff")
  #cell_line_color_grad[["MEL-JUSO"]] <- c("#3d0067","#9900ff", "#d79aff")
  #cell_line_color_grad[["SK-MEL-2"]] <- c("#3d0067","#9900ff", "#d79aff")
  #cell_line_color_grad[["SK-MEL-30"]] <- c("#3d0067","#9900ff", "#d79aff")
} else{
  cell_line_color_grad <- hash()
  for (cell_name in cellLinesToUse){
    cell_line_color_grad[[cell_name]] <- c("#3d0067","#9900ff", "#d79aff")
  }
}
...
patient_colors <- rep(c("darkblue","blue","orange","yellow", "black","cyan","grey","darkgreen","purple","darkgoldenrod3"),length(belva_Doses_To_Use))
...
for (j in 1:dose_count){
  ...
  cplot[[length(cplot)+1]] <- ggplot()+ ggtitle(sprintf("%s\n%s",belva_Doses_To_Use[j],cobi_Doses_To_Use[j])) + ylim(min(min_effect,0),max_effect+.01)+ theme_classic() 
  if (gtf$short =='GR'){
    cplot[[length(cplot)]] <- cplot[[length(cplot)]] + geom_hline(yintercept=0, linetype="dashed", color = "black")
  }
  for (cell_line_name in keys(cell_line_color_grad)){
    ...
    print(length(projected_doses_sc_filtered$Combo))
    cplot[[length(cplot)]] <- cplot[[length(cplot)]] +geom_quasirandom(data=projected_doses_sc_filtered, aes(x = CellLineName, y = Combo,color = time),bandwidth = bandwidth)+scale_color_gradientn(colours = cell_line_color_grad[[cell_line_name]])
    if (unique_cld_color){
      cplot[[length(cplot)]] <- cplot[[length(cplot)]]+new_scale_color()
    }
  }
  #cplot[[length(cplot)+1]] <- ggplot(data=projected_doses_filtered) + ggtitle(sprintf("%s\n%s",belva_Doses_To_Use[j],cobi_Doses_To_Use[j])) + ylim(min(min_effect,0),max_effect+.01) + geom_quasirandom(aes(x = CellLineName, y = Combo,color = time, fill=CellLineName)) + theme_classic() #+ stat_summary(fun.y=mean, geom="point", size=4, color="black")
}
...
for (single_agent_ID in single_agents){
  ...
  #cplot[[length(cplot)+1]] <- ggplot(data=single_agent_project_df_filtered, aes(x = CellLineName, y = Combo,color = time, fill=time))+ggtitle(single_agent_ID) + ylim(min(min_effect,0),max_effect+.01) + geom_quasirandom()+ stat_summary(fun.y=mean, geom="point", size=4, color="black") + theme_classic()
  cplot[[length(cplot)+1]] <- ggplot()+ggtitle(paste("Single Agent:\n",single_agent_ID))+ ylim(min(min_effect,0),max_effect+.01)+ theme_classic() 
  if (gtf$short =='GR'){
    cplot[[length(cplot)]] <- cplot[[length(cplot)]] + geom_hline(yintercept=0, linetype="dashed", color = "black")
  }
  for (cell_line_name in keys(cell_line_color_grad)){
    
    projected_doses_sc_filtered <- filter(single_agent_project_df_filtered,CellLineName == cell_line_name)
    print(length(projected_doses_sc_filtered$Combo))
    #print(projected_doses_sc_filtered)
    #geom_quasirandom
    cplot[[length(cplot)]] <- cplot[[length(cplot)]] +geom_quasirandom(data=projected_doses_sc_filtered, aes(x = CellLineName, y = Combo,color = time),bandwidth = bandwidth)+scale_color_gradientn(colours = cell_line_color_grad[[cell_line_name]])
    if (unique_cld_color){
      cplot[[length(cplot)]] <- cplot[[length(cplot)]]+new_scale_color()
...
print(grid.arrange(grobs = cplot, ncol=ncol))
dev.off()
...
for (j in 1:dose_count){
  ...
  patients_to_map <- unique(projected_doses_filtered$ID)[violin_patients]
  projected_doses_filtered <- filter(projected_doses_filtered,projected_doses_filtered$ID %in% patients_to_map)
  print(sprintf("%s\n%s",belva_Doses_To_Use[j],cobi_Doses_To_Use[j]))
...
## Generates single agent Bliss Excess violin plots
for (single_agent_ID in single_agents){
  #cycle_time <- get_cycle_length(single_agent_ID)
  cycle_time <- 48
  if (single_agent_ID %in% single_agent_project_df$Dose_ID){
  ...
  patients_to_map <- na.omit(patients_to_map)
  single_agent_project_df_filtered <- filter(single_agent_project_df_filtered,single_agent_project_df_filtered$ID %in% patients_to_map)
  bplot[[length(bplot)+1]] <- ggplot()+ggtitle(paste("Single Agent:\n",single_agent_ID))+ ylim(min(min_effect,0),max_effect+.01)+ theme_classic() + geom_hline(yintercept=0, linetype="dashed", color = "black")
  for (cell_line_name in keys(cell_line_color_grad)){
    projected_doses_sc_filtered <- filter(single_agent_project_df_filtered,CellLineName == cell_line_name)
    projected_doses_sc_filtered$Bliss_Excess <- projected_doses_sc_filtered$Bliss - projected_doses_sc_filtered$Combo
    print(length(projected_doses_sc_filtered$Combo))
    bplot[[length(bplot)]] <- bplot[[length(bplot)]] +geom_quasirandom(data=projected_doses_sc_filtered, aes(x = CellLineName, y = Bliss_Excess,color = time),bandwidth = bandwidth)+scale_color_gradientn(colours = cell_line_color_grad[[cell_line_name]])
    if (unique_cld_color){
      bplot[[length(bplot)]] <- bplot[[length(bplot)]]+new_scale_color()
    }
  }
...
nrow <- length(bplot)/4
pdf(file.path(cwd, figures_dir, 'PK_variability' ,file_res), width= 10 * ncol , height=5 * nrow  )  
print(grid.arrange(grobs = bplot, ncol=ncol))
dev.off()
...
for (j in 1:dose_count){
  ...
  for (patient_ID_ind in 1:length(patients_to_map)){
    projected_curve <- filter(projected_doses_filtered,ID == patients_to_map[patient_ID_ind])
    pkplot[[patient_ID_ind+(j-1)*length(patients_to_map)]]  <- ggplot() + geom_path(data = projected_curve,aes(x=time, y=free_Concentration_2), linewidth = 1,linetype="dashed")+ggtitle(sprintf("Patient ID: %d\n%s\n%s",patients_to_map[patient_ID_ind],belva_Doses_To_Use[j],cobi_Doses_To_Use[j])) + theme(
